 ...
 */
new KyeeModule()
    .group("kyee.quyiyuan.myWallet.clinicPaymentReviseHos.controller")
    .require(["kyee.quyiyuan.myWallet.clinicPaymentRevise.service",
        ...
        "kyee.quyiyuan.myWallet.clinicPaidHos.controller"])
    .type("controller")
    .name("ClinicPaymentReviseHosController")
    .params(["PatientCardService","CenterUtilService","$scope", "$rootScope", "$state", "$ionicHistory", "ClinicPaymentReviseService", "KyeeMessageService",
        ...
        "PaidRecordService"])
    .action(function (PatientCardService,CenterUtilService,$scope, $rootScope, $state, $ionicHistory, ClinicPaymentReviseService, KyeeMessageService,
                      PayOrderService, KyeeListenerRegister, KyeeViewService, CacheServiceBus, QueryHisCardService,
                      KyeeUtilsService, KyeeI18nService, HospitalSelectorService, $compile, $timeout, $ionicScrollDelegate,
                      AuthenticationService, $ionicListDelegate,ClinicPaidService,CommPatientDetailService,AppointmentRegistDetilService,
                      PaidRecordService) {
                    ...
                    if (rechargeInfo[j] && rechargeInfo[j].CARD_TYPE && rechargeInfo[j].CARD_TYPE == 0 && rechargeInfo[j].CARD_NO != undefined && rechargeInfo[j].CARD_NO.substring(0, 1) == "Q") {
                        ...
                        if(currentCardNo){
                            ...
                            if(rechargeInfo[j].IS_DEFAULT==1){
                                ...
                                if(rechargeInfo[j].CARD_NAME === '电子健康卡'){
            ...
            if($scope.pickerItems){
                for (var i = 0; i < $scope.pickerItems.length; i++) {
                    if ($scope.currentCustomPatient.CARD == $scope.pickerItems[i].value) {
                        ...
                        if($scope.pickerItems[i].name === '电子健康卡'){
                            PayOrderService.isHealthCard = true;
                        }else{
                            PayOrderService.isHealthCard = false;
                        }
        ...
        var init = function(data,success,prompt){
             ...
             * 初始化查询方式
             */
            $scope.QUERY_PAY_TYPE = data.QUERY_PAY_TYPE;
            ClinicPaidService.paymentPayhisParms.QUERY_PAY_TYPE = data.QUERY_PAYMENT_PAYRESTYPE;//将查询方式传入Service
            //QUERY_PAY_TYPE: 1:就诊卡；2：就诊卡+姓名；3：身份证+姓名；4：手机号+姓名; 5:查询方式选择了1和2，但是仅支持虚拟卡
            if(data.QUERY_PAY_TYPE==1||data.QUERY_PAY_TYPE==2 || data.QUERY_PAYMENT_PAYRESTYPE==1 || data.QUERY_PAYMENT_PAYRESTYPE==2){
                  //获取卡列表
                if(data.CARD_LIST){
                     // 填充卡
            ...
            //PAYMENT_FLAG：门诊待缴费；PAYMENT_PAYHIS_FLAG：门诊已缴费；PAY_SUBMIT_FLAG：门诊缴费
            ClinicPaidService.paymentPayhisParms.PAYMENT_FLAG = data.PAYMENT_FLAG;
            ClinicPaidService.paymentPayhisParms.PAYMENT_PAYHIS_FLAG = data.PAYMENT_PAYHIS_FLAG;
            ClinicPaidService.paymentPayhisParms.PAY_SUBMIT_FLAG = data.PAY_SUBMIT_FLAG;

            if(data && (data.PAYMENT_PAYHIS_FLAG == "1" || data.PAY_SUBMIT_FLAG == "1")){
                $scope.isShowHisData = true;
            }else{
                $scope.isShowHisData = false;
            }
            if(data && (data.PAYMENT_FLAG == "0" && data.PAY_SUBMIT_FLAG == "0")){
                $scope.isPermission = false;
            }else{
                $scope.isPermission = true;
            }
                ...
                if ((data.REG && data.REG.length > 0)||(data.PAY && data.PAY.length > 0)) {
                    ...
                    if (data.PAY && data.PAY.length > 0) {
                        ...
                        //待缴费
                            ...
                            //全选时直接显示总金额
                            var pay = data.PAY[0];
                            if(pay.PAYMENT_INFO&&pay.PAYMENT_INFO.length>0){
                                $scope.last_index= pay.PAYMENT_INFO.length-1;
                            }
        ...
        $scope.doRefresh = function (isInit) {
            ...
            $scope.roundTip = undefined;
            getPatientInfo();
            ClinicPaymentReviseService.PHONE_NUMBER = $scope.phoneNumber;
        ...
        $scope.paidRecGoDetail = function(paidItem){
            ...
            if(paidItem.DEPT_NAME){
                ...
                AppointmentRegistDetilService.ROUTE_STATE = "clinic_payment_hos";
