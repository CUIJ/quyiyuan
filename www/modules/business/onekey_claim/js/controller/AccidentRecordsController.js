 ...
 * 产品名称：quyiyuan
 * 创建者：程铄闵
 * 创建时间：2016年10月12日09:22:34
 * 创建原因：添加事故资料控制器
 * 任务号：KYEEAPPC-8137
 */
new KyeeModule()
    .group("kyee.quyiyuan.accidentRecords.controller")
    .require(["kyee.quyiyuan.accidentRecords.service",
        "kyee.quyiyuan.clinicClaim.controller"])
    .type("controller")
    .name("AccidentRecordsController")
    .params(["$scope", "$state","KyeeMessageService","KyeeI18nService","KyeeListenerRegister",
        "KyeeCameraService","KyeeDeviceInfoService","AccidentRecordsService","MedicalRecordsService","$location"])
    .action(function ($scope, $state,KyeeMessageService,KyeeI18nService,KyeeListenerRegister,
        ...
        var init = function(){
            ...
            //获取上一页照片数据
            $scope.picTotal = MedicalRecordsService.pictureTotal;
        };
        ...
        $scope.nextStep = function(){
            AccidentRecordsService.pictureTotal = $scope.picSource.length - 1;
            $state.go('clinic_claim');
        };
        ...
        $scope.addPicture = function () {
            if(isWeiXin())
            {
                ...
                KyeeMessageService.actionsheet({
                    buttons: [
                        {
                            text: KyeeI18nService.get("accident_records.takePhotos","拍照")
                        },
                        {
                            text: KyeeI18nService.get("accident_records.fromPhotos","从相册选择")
                        }
                    ],
